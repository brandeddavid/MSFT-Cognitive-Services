...
with open('SolarPVContacts.csv', 'a', newline = '') as l:
    ...
    with open('SolarPVPlants.csv', 'r') as file:
        ...
        for row in reader:
            ...
            searchphrase = row[14] + ' intitle:contact'
                ...
                try:
                    ...
                    for item in jsonResponse['webPages']['value']:
                        ...
                        if jsonResponse['webPages']['value'].index(item) < 1:
                            ...
                            writer.writerow({'PLANT NAME':row[0], 'OPERATING COMPANY':row[14], 'WEBSITE NAME':item['name'], 'CONTACT URL':item['url']})
