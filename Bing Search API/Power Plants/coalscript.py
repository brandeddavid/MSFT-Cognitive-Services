...
with open('150.csv', 'a', newline = '') as l:
    ...
    with open('CoalPlants.csv', 'r') as file:
        ...
        for row in reader:
            ...
            searchphrase = row[16] + ' intitle:contact'
                ...
                try:
                    ...
                    for item in jsonResponse['webPages']['value']:
                        ...
                        if jsonResponse['webPages']['value'].index(item) < 1:
                            ...
                            writer.writerow({'PLANT NAME':row[0], 'OPERATING COMPANY':row[16], 'WEBSITE NAME':item['name'], 'CONTACT URL':item['url']})
