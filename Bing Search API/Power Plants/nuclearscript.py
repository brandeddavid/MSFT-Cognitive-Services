...
with open('NuclearContacts.csv', 'a', newline = '') as l:
    ...
    with open('NuclearPlants.csv', 'r') as file:
        ...
        for row in reader:
            ...
            searchphrase = row[13] + ' intitle:contact'
                ...
                try:
                    ...
                    for item in jsonResponse['webPages']['value']:
                        ...
                        if jsonResponse['webPages']['value'].index(item) < 1:
                            ...
                            writer.writerow({'PLANT NAME':row[0], 'OPERATING COMPANY':row[13], 'WEBSITE NAME':item['name'], 'CONTACT URL':item['url']})
